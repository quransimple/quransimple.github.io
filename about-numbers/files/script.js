...
function setTable() {
	...
	function getQWValueNumber() {
		let QW_numberValue = document.querySelector("#QW-numberValue");
		let chosenWords = "";
		for(let i = 0; i < quranWords_numberValue.length; i++) {
			if(quranWords_numberValue[i] == inputElem.value) {
				chosenWords += quranWords[i] + " ";
			}
		}
		if(chosenWords != "") {
			QW_numberValue.innerHTML = chosenWords;
		} else {
			QW_numberValue.innerHTML = "";
		}
	}
	getQWValueNumber();
